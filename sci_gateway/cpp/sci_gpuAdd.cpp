...
int sci_gpuAdd(char *fname)
{
    ...
    try
    {
        ...
        if (inputType_A == sci_pointer)
        {
            ...
            {
                throw "gpuAdd : Bad type for input argument #1: Variables created with GPU functions expected.";
            }

            if (useCuda() && gpuPtrA->getGpuType() != GpuPointer::CudaType)
            {
                throw "gpuAdd : Bad type for input argument #1: A Cuda pointer expected.";
            }

            if (useCuda() == false && gpuPtrA->getGpuType() != GpuPointer::OpenCLType)
            {
                throw "gpuAdd : Bad type for input argument #1: A OpenCL pointer expected.";
            }
        }
        else if (inputType_A == sci_matrix)
        {
            if (isVarComplex(pvApiCtx, piAddr_A))
            {
                ...
                if (!useCuda())
                {
                    Scierror(999, "gpuAdd: not implemented with OpenCL.\n");
                }
#endif
            }
            else
        ...
        {
            throw "gpuAdd : Bad type for input argument #1: A GPU or CPU matrix expected.";
        }
        ...
        if (inputType_B == sci_pointer)
        {
            ...
            {
                throw "gpuAdd : Bad type for input argument #2: Variables created with GPU functions expected.";
            }

            if (useCuda() && gpuPtrB->getGpuType() != GpuPointer::CudaType)
            {
                throw "gpuAdd : Bad type for input argument #2: A Cuda pointer expected.";
            }

            if (useCuda() == false && gpuPtrB->getGpuType() != GpuPointer::OpenCLType)
            {
                throw "gpuAdd : Bad type for input argument #2: A OpenCL pointer expected.";
            }
        }
        ...
        {
            throw "gpuAdd : Bad type for input argument #2: A GPU or CPU matrix expected.";
        }

        if ((gpuPtrA->getSize() > 1 && gpuPtrB->getSize() > 1) && (gpuPtrA->getCols() != gpuPtrB->getCols() || gpuPtrA->getRows() != gpuPtrB->getRows()))
        {
            throw "gpuAdd : Bad size for inputs arguments: Incoherent addition.";
        }
