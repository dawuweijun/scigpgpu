...
int sci_gpuMax(char *fname)
{
    ...
    try
    {
        ...
        if (inputType_A == sci_pointer)
        {
            ...
            {
                throw "gpuMax : Bad type for input argument #1: Variables created with GPU functions expected.";
            }

            if (useCuda() && gpuPtrA->getGpuType() != GpuPointer::CudaType)
            {
                throw "gpuMax : Bad type for input argument #1: A Cuda pointer expected.";
            }

            if (useCuda() == false && gpuPtrA->getGpuType() != GpuPointer::OpenCLType)
            {
                throw "gpuMax : Bad type for input argument #1: A OpenCL pointer expected.";
            }
        ...
        else if (inputType_A == sci_matrix)
        {
            if (isVarComplex(pvApiCtx, piAddr_A))
            {
                ...
                if (!useCuda())
                {
                    Scierror(999, "gpuMax: not implemented with OpenCL.\n");
                }
#endif
            }
            else
        ...
        {
            throw "gpuMax : Bad type for input argument #1: A GPU or CPU matrix expected.";
        }
        ...
        if (nbInputArgument(pvApiCtx) == 2)
        {
            if (inputType_B == sci_pointer)
            {
                ...
                {
                    throw "gpuMax : Bad type for input argument #2: Variables created with GPU functions expected.";
                }

                if (useCuda() && gpuPtrB->getGpuType() != GpuPointer::CudaType)
                {
                    throw "gpuMax : Bad type for input argument #2: A Cuda pointer expected.";
                }

                if (useCuda() == false && gpuPtrB->getGpuType() != GpuPointer::OpenCLType)
                {
                    throw "gpuMax : Bad type for input argument #2: A OpenCL pointer expected.";
                }

                if (gpuPtrB->getRows() != rows || gpuPtrB->getCols() != cols)
                {
                    throw "gpuMax : Bad type for input argument #2: Same size expected.";
                }
            }
            ...
            {
                throw "gpuMax : Bad type for input argument #2: A GPU or CPU matrix expected.";
            }
        }
        ...
        if (!useCuda())
        {
            throw "gpuMax: not implemented with OpenCL.";
        }
#endif
        ...
        if (gpuPtrB == NULL)
        {
            if (gpuPtrA->isGpuComplex())
            {
                ...
                sciprint("To find the max value of complex matrix, the function performs operation abs(real)+abs(imaginary).\n");

#ifdef WITH_CUDA
                if (useCuda())
                {
                    // Performs operation using cublas
                    cuDoubleComplex res = ((PointerCuda*)gpuPtrA)->getComplexMax();
        ...
        else
        {
            ...
            if (useCuda())
            {
                ...
                // Performs operation
                cudaElemMax(dynamic_cast<PointerCuda*>(gpuPtrA), dynamic_cast<PointerCuda*>(gpuPtrB), dynamic_cast<PointerCuda*>(gpuPtrRes));
            }
#endif
