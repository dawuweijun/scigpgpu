...
int sci_gpuMult(char *fname)
{
    ...
    try
    {
        ...
        if (inputType_A == sci_pointer)
        {
            ...
            {
                throw "gpuMult : Bad type for input argument #1: Variables created with GPU functions expected.";
            }

            if (useCuda() && gpuPtrA->getGpuType() != GpuPointer::CudaType)
            {
                throw "gpuMult : Bad type for input argument #1: A Cuda pointer expected.";
            }

            if (useCuda() == false && gpuPtrA->getGpuType() != GpuPointer::OpenCLType)
            {
                throw "gpuMult : Bad type for input argument #1: A OpenCL pointer expected.";
            }
        }
        else if (inputType_A == sci_matrix)
        {
            if (isVarComplex(pvApiCtx, piAddr_A))
            {
                ...
                if (!useCuda())
                {
                    throw "gpuMult: not implemented with OpenCL.";
                }
#endif
            }
            else
        ...
        {
            throw "gpuMult : Bad type for input argument #1: A GPU or CPU matrix expected.";
        }
        ...
        if (inputType_B == sci_pointer)
        {
            ...
            {
                throw "gpuMult : Bad type for input argument #2: Variables created with GPU functions expected.";
            }

            if (useCuda() && gpuPtrB->getGpuType() != GpuPointer::CudaType)
            {
                throw "gpuMult : Bad type for input argument #2: A Cuda pointer expected.";
            }

            if (useCuda() == false && gpuPtrB->getGpuType() != GpuPointer::OpenCLType)
            {
                throw "gpuMult : Bad type for input argument #2: A OpenCL pointer expected.";
            }
        }
        else if (inputType_B == sci_matrix)
        {
            if (isVarComplex(pvApiCtx, piAddr_B))
            {
                ...
                if (!useCuda())
                {
                    Scierror(999, "gpuMult: not implemented with OpenCL.\n");
                }
#endif
            }
            else
        ...
        {
            throw "gpuMult : Bad type for input argument #2: A GPU or CPU matrix expected.";
        }
        ...
        if (gpuPtrA->getCols() != gpuPtrB->getRows() && gpuPtrA->getSize() > 1 && gpuPtrB->getSize() > 1)
        {
            throw "gpuMult : Bad size for inputs arguments: Incoherent multiplication.";
        }
