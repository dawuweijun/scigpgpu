...
int sci_gpuExtract(char *fname)
{
    ...
    try
    {
        ...
        if (inputType_A == sci_pointer)
        {
            ...
            {
                throw "gpuExtract : Bad type for input argument #1: Variables created with GPU functions expected.";
            }

            if (useCuda() && gpuPtrA->getGpuType() != GpuPointer::CudaType)
            {
                throw "gpuExtract : Bad type for input argument #1: A Cuda pointer expected.";
            }

            if (useCuda() == false && gpuPtrA->getGpuType() != GpuPointer::OpenCLType)
            {
                throw "gpuExtract : Bad type for input argument #1: A OpenCL pointer expected.";
            }
        }
        else if (inputType_A == sci_matrix)
        {
            ...
            if (isVarComplex(pvApiCtx, piAddr_A))
            {
                ...
                if (!useCuda())
                {
                    Scierror(999, "gpuExtract: not implemented with OpenCL.\n");
                }
#endif
            }
            else
        ...
        {
            throw "gpuExtract : Bad type for input argument #1: A GPU or CPU matrix expected.";
        }
        ...
        if (inputType_R == sci_pointer)
        {
            ...
            {
                throw "gpuExtract : Bad type for input argument #2: Variables created with GPU functions expected.";
            }

            if (useCuda() && gpuPtrR->getGpuType() != GpuPointer::CudaType)
            {
                throw "gpuInterp : Bad type for input argument #3: A Cuda pointer expected.";
            }

            if (useCuda() == false && gpuPtrR->getGpuType() != GpuPointer::OpenCLType)
            {
                throw "gpuInterp : Bad type for input argument #3: A OpenCL pointer expected.";
            }

            if (gpuPtrR->isGpuComplex())
            {
                throw "gpuExtract : Bad type for input argument #2: A real scalar or matrix expected.";
            }
        }
        ...
        {
            throw "gpuExtract : Bad type for input argument #2: A GPU or CPU scalar or matrix expected.";
        }
        ...
        if (nbInputArgument(pvApiCtx) == 3)
        {
            ...
            {
                throw "gpuExtract : Wrong size for input argument #2: Colomn vector expected.";
            }
            ...
            if (inputType_C == sci_pointer)
            {
                ...
                {
                    throw "gpuExtract : Bad type for input argument #3: Variables created with GPU functions expected.";
                }

                if (useCuda() && gpuPtrC->getGpuType() != GpuPointer::CudaType)
                {
                    throw "gpuInterp : Bad type for input argument #3: A Cuda pointer expected.";
                }

                if (useCuda() == false && gpuPtrC->getGpuType() != GpuPointer::OpenCLType)
                {
                    throw "gpuInterp : Bad type for input argument #3: A OpenCL pointer expected.";
                }
                ...
                {
                    throw "gpuExtract : Bad type for input argument #3: A real scalar or matrix expected.";
                }
            }
            else if (inputType_C == sci_matrix)
            {
                double* h = NULL;
                if (isVarComplex(pvApiCtx, piAddr_C))
                {
                    throw "gpuExtract : Bad type for input argument #3: A real scalar or matrix expected.";
                }
                else
            ...
            {
                throw "gpuExtract : Bad type for input argument #3: A GPU or CPU scalar or matrix expected.";
            }

            if (gpuPtrC->getRows() != 1)
            {
                throw "gpuExtract : Wrong size for input argument #3: Row vector expected.";
            }

#ifdef WITH_OPENCL
            if (!useCuda())
            {
                throw "gpuExtrat : not yet implemented with OpenCL.";
            }
#endif
        ...
        GpuPointer* gpuOut = gpuPtrA->extract(gpuPtrPos);
        if (gpuOut == NULL)
        {
            throw "gpuExtract : Invalid index.";
        }
