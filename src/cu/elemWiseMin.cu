...
__global__ void elementWiseMin(double *odata, double* idataA, double* idataB, int width, int height)
{
...
__global__ void elementWiseZMin(cuDoubleComplex *odata, cuDoubleComplex* idataA, cuDoubleComplex* idataB, int width, int height)
{
    unsigned int xIndex = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int yIndex = blockDim.y * blockIdx.y + threadIdx.y;
    unsigned int index  = yIndex + height * xIndex;
...
__global__ void elementWiseZDMin(cuDoubleComplex *odata, cuDoubleComplex* idataA, double* idataB, int width, int height)
{
    unsigned int xIndex = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int yIndex = blockDim.y * blockIdx.y + threadIdx.y;
    unsigned int index  = yIndex + height * xIndex;
...
cudaError_t cudaMinElementwise(double* d_inputA, double* d_inputB, double* d_output, int rows, int cols)
{
    ...
    try
    {
        ...
        elementWiseMin<<<grid, block>>>(d_output, d_inputA, d_inputB, cols, rows);
...
cudaError_t cudaZMinElementwise(cuDoubleComplex* d_inputA, cuDoubleComplex* d_inputB, cuDoubleComplex* d_output, int rows, int cols)
{
    ...
    try
    {
        ...
        elementWiseZMin<<<grid, block>>>(d_output, d_inputA, d_inputB, cols, rows);
...
cudaError_t cudaZDMinElementwise(cuDoubleComplex* d_inputA, double* d_inputB, cuDoubleComplex* d_output, int rows, int cols)
{
    ...
    try
    {
        ...
        elementWiseZDMin<<<grid, block>>>(d_output, d_inputA, d_inputB, cols, rows);
