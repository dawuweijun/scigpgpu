...
__global__ void idmin_kernel(double* d, int elems, double* result)
{
...
cudaError_t cudaIdmin(int elems, double* d, double* res)
{
	double* input  	= NULL;
	double* output 	= NULL;
	...
	try
	{
		...
		while(true)
		{
			...
			dim3 block(threadMax, 1, 1);
		   	...
		   	idmin_kernel<<<grid, block, threadMax*8>>>(input,elems,output);
