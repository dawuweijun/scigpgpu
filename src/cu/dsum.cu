...
__global__ void dsum_kernel(double* d, int elems, double* result)
{
	int posInGrid  = blockIdx.x  * blockDim.x + threadIdx.x;
   	extern __shared__ double accumResult[];

	accumResult[threadIdx.x] = 0.0;
...
cudaError_t cudaDsum(int elems, double* d, double* res)
{
	double* input  	= NULL;
	double* output 	= NULL;
	...
	try
	{
		...
		while(true)
		{
			...
			dim3 block(threadMax, 1, 1);
		   	...
		   	dsum_kernel<<<grid, block, threadMax*8>>>(input,elems,output);
