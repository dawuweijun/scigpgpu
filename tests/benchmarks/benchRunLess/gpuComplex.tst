...
function bench()
    ...
    abs_path = get_absolute_file_path("gpuComplex.tst");
    fileName = abs_path + "benchData/complex.h5";
    ...
    for i = 1:n,
        ...
        for j = 1:m
            tic(); dgpu = gpuComplex(da,db); t = toc();
            dgpu = gpuFree(dgpu);
            computeTime(i,4) = computeTime(i,4) + t;
        end
        dgpu = gpuComplex(da,db);
    ...
    saveMyBench(fileName,"Scilab - Create a complex with two real.","Data Size (Thousand of elements)","Computation Time (milliseconds)",..
