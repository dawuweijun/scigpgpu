...
function bench()
    ...
    abs_path = get_absolute_file_path("gpuTransposeOnce.tst");
    fileName = abs_path + "benchData/transpose.h5";
    ...
    for i = 1:length(XSize),
        ...
        tic();b = a'; computeTime(i,1) = toc();

        tic();da    = gpuSetData(a);            computeTime(i,3) = toc();
        tic();dres  = gpuTranspose(da);   computeTime(i,4) = toc();
        gpuFree(da);
    ...
    saveMyBench(fileName,"Scilab - Transpose once matrix","Data Size (Million of elements)","Computation Time (seconds)",dataSize,computeTime,..
