...
function bench()
    ...
    abs_path = get_absolute_file_path("gpuDotMult.tst");
    fileName = abs_path + "benchData/dotmult.h5";
    ...
    for i = 1:n,
        ...
        for j = 1:m
            tic(); dgpu = gpuDotMult(da,db); t = toc();
            gpuFree(dgpu);
            computeTime(i,4) = computeTime(i,4) + t;
        end
        dgpu = gpuDotMult(da,db);
    ...
    saveMyBench(fileName,"Scilab - Matrix dot multiplication","Data Size (Million of elements)","Computation Time (milliseconds)",dataSize,computeTime,..
