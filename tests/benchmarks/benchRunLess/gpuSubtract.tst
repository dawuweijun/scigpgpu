...
function bench()
    ...
    abs_path = get_absolute_file_path("gpuSubtract.tst");
    fileName = abs_path + "benchData/subtraction.h5";
    ...
    for i = 1:n,
        ...
        for j = 1:m
            tic(); dgpu = gpuSubtract(da,db); t = toc();
            dgpu = gpuFree(dgpu);
            computeTime(i,4) = computeTime(i,4) + t;
        end
        dgpu = gpuSubtract(da,db);
    ...
    saveMyBench(fileName,"Scilab - Matrix subtraction","Data Size (Million of elements)","Computation Time (milliseconds)",dataSize,computeTime,..
